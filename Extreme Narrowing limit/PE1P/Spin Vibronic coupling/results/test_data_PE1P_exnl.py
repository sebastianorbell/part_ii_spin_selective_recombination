...
data = np.loadtxt('PE1P_exnl_spin_vib.txt',delimiter=',')
...
plt.plot(1.0/pe1p_k_sin[:,0],((np.exp(pe1p_k_sin[:,1])*np.sqrt(pe1p_k_sin[:,0])*(1.0/1.73e8))),'o--',label='Ks experimental')
...
plt.plot(data[:,4],data[:,0],'g^--',label = 'x')
