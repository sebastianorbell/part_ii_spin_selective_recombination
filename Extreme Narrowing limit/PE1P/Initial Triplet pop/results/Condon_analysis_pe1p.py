...
def fitting_extra_terms(a_t,b_t,a_s,b_s,j0,c):
    ...
    for index, temp in enumerate(temperature):
       ...
       ks[index],kt[index],jcalc[index] = rate_functions_model_3(a_t,b_t,a_s,b_s,j0,c,temp)
...
def fitting_extra_terms_print(a_t,b_t,a_s,b_s,j0,c):
    ...
    j_mean = np.sum(J)/6.0
    fac = 10000.0
    val =  (np.sum((jcalc - J)*(jcalc - J))/np.sum((j_mean - J)*(j_mean - J))) + fac*(np.sum((ks-ks_exp)*(ks-ks_exp))/np.sum((ks_exp_mean-ks_exp)*(ks_exp_mean-ks_exp))) + fac*(np.sum((kt-kt_exp)*(kt-kt_exp))/np.sum((kt_exp_mean-kt_exp)*(kt_exp_mean-kt_exp)))
    ...
    print('-------------------------------------------------------------------')
    print("PE1P")
    print('a_t,b_t,a_s,b_s,j0,c')
    print(a_t,b_t,a_s,b_s,j0,c)
    
    return val

bnds = [(1.0e0,1.0e6),(1.0e-6,1.0e2),(1.0e-2,1.0e2),(1.0e-2,1.0e2),(1.0e-4,5.0e1),(-5.0e1,5.0e1)]
...
#fitting_extra_terms_print(1062.627572148624, 26.327647938027056, 1.4438243063892096, 9.981557507671411, 15, -0.002379555303484542)
